...
// ===== STORAGE =====
function save() {
  // පරණ localStorage එක වෙනුවට Firebase එකට දත්ත යවනවා
  db.ref('projectData').set(state);
}

function load() {
  // database එකේ 'projectData' කියන තැනට listener එකක් දානවා
  db.ref('projectData').on('value', (snapshot) => {
    const data = snapshot.val();
    if (data) {
      state = data; // state එක අලුත් දත්ත වලින් update කරනවා
      renderAll();  // screen එක අලුතින් අඳිනවා (UI refresh වෙනවා)
      updateBadges();
    }
  });
}
...
// ===== INIT =====
window.onload = () => {
  load(); // මෙතනදී තමයි Firebase එකත් එක්ක connect වෙලා data ටික ගන්නේ
  initColorSwatches();
  checkAuth();
  drawLoginCanvas();
};
...
function selectColor(containerId, color, el) {
  ...
  if (containerId === 'colorSwatches') setupSelectedColor = color;
...
function doLogin() {
  ...
  if (state.members.length === 0) { err.textContent = 'No members yet. Use setup mode first.'; return; }
...
function doSetupSave() {
  ...
  const member = { id: uid(), name, email, color: setupSelectedColor, initials: makeInitials(name) };
...
function showSection(name, btn) {
  ...
  if (name === 'chat') {
    setTimeout(() => { const ca = document.getElementById('chatArea'); ca.scrollTop = ca.scrollHeight; }, 50);
  }
...
function renderDashboard() {
  ...
  // Recent Tasks
  const rt = document.getElementById('recentTasksList');
  const recentTasks = state.tasks.slice(-4).reverse();
  rt.innerHTML = recentTasks.length ? recentTasks.map(t => `
    <div class="recent-item" onclick="openTaskModal('${t.id}')">
      ...
      <div class="recent-label">${t.title}</div>
      <div class="recent-meta">${statusLabel(t.status)}</div>
    </div>
  `).join('') : '<div class="recent-empty">No tasks yet</div>';

  // Recent Milestones
  const rm = document.getElementById('recentMilestones');
  const upcoming = state.milestones.filter(m => m.status !== 'done').slice(0,3);
  rm.innerHTML = upcoming.length ? upcoming.map(m => `
    <div class="recent-item" onclick="openMilestoneModal('${m.id}')">
      ...
      <div class="recent-label">${m.name}</div>
      <div class="recent-meta">${m.end ? formatDate(m.end) : ''}</div>
    </div>
  `).join('') : '<div class="recent-empty">No milestones yet</div>';

  // Recent Papers
  const rp = document.getElementById('recentPapers');
  const papers = state.papers.slice(-3).reverse();
  rp.innerHTML = papers.length ? papers.map(p => `
    <div class="recent-item" onclick="openPaperModal('${p.id}')">
      ...
      <div class="recent-label">${p.title}</div>
      <div class="recent-meta">${p.year||''}</div>
    </div>
  `).join('') : '<div class="recent-empty">No papers yet</div>';

  // Recent Meetings
  const rmt = document.getElementById('recentMeetings');
  const meetings = state.meetings.slice(-3).reverse();
  rmt.innerHTML = meetings.length ? meetings.map(m => `
    <div class="recent-item" onclick="openMeetingModal('${m.id}')">
      <div class="recent-dot" style="background:var(--violet)"></div>
      <div class="recent-label">${m.title}</div>
      <div class="recent-meta">${m.date ? formatDate(m.date) : ''}</div>
    </div>
...
function updateDeadlineChip() {
  const chip = document.getElementById('deadlineChip');
...
function renderTaskCard(t) {
  ...
  let dueClass = '';
  if (t.due) {
    if (t.due < today) dueClass = 'overdue';
...
function filterTasksByAssignee() {
  assigneeFilter = document.getElementById('assigneeFilter').value;
  renderTasks();
}
...
function openTaskModal(id) {
  ...
  sel.innerHTML = '<option value="">Unassigned</option>' +
    state.members.map(m => `<option value="${m.id}" ${t&&t.assignee===m.id?'selected':''}>${m.name}</option>`).join('');
...
function saveTask() {
  ...
  if (editingId) {
    ...
    state.tasks.push({ id: uid(), createdBy: currentUser.id, createdAt: new Date().toISOString(), ...taskData });
  }
...
function renderTimeline() {
  ...
  if (!state.milestones.length) {
    wrap.innerHTML = '<div class="timeline-empty">No milestones yet. Click "+ Add Milestone" to get started.</div>';
    return;
  }
  const sorted = [...state.milestones].sort((a,b) => (a.end||'').localeCompare(b.end||''));
  wrap.innerHTML = sorted.map(m => `
    <div class="milestone-card ${m.status}" onclick="openMilestoneModal('${m.id}')">
      ...
      <div class="milestone-dates">
        ${m.start ? `From ${formatDate(m.start)}` : ''}
        ${m.end ? ` · Due ${formatDate(m.end)}` : ''}
      </div>
...
function saveMilestone() {
  ...
  if (editingId) {
    const idx = state.milestones.findIndex(m => m.id === editingId);
    if (idx > -1) state.milestones[idx] = { ...state.milestones[idx], ...data };
  } else {
    state.milestones.push({ id: uid(), ...data });
  }
  save();
  closeAllModals();
  renderTimeline();
...
function getAddedByName(id) {
  const m = state.members.find(m => m.id === id);
  return m ? 'Added by ' + m.name : '';
}
...
function savePaper() {
  ...
  if (editingId) {
    const idx = state.papers.findIndex(p => p.id === editingId);
    if (idx > -1) state.papers[idx] = { ...state.papers[idx], ...data };
  } else {
    state.papers.push({ id: uid(), addedBy: currentUser.id, addedAt: new Date().toISOString(), ...data });
  }
  save();
  closeAllModals();
  renderResearch();
...
function renderMeetings() {
  ...
  if (!state.meetings.length) {
    list.innerHTML = '<div style="color:var(--text3);text-align:center;padding:60px">No meetings yet. Click "+ Add Meeting" to add notes.</div>';
    return;
  }
  ...
  list.innerHTML = sorted.map(m => `
    <div class="meeting-card" onclick="openMeetingModal('${m.id}')">
      ...
      ${m.actions ? `
        <div class="meeting-actions-text">
          <div class="meeting-actions-label">Action Items</div>
          ${m.actions.slice(0,150)}${m.actions.length>150?'…':''}
        </div>
      ` : ''}
    </div>
  `).join('');
...
function saveMeeting() {
  ...
  const data = {
    ...
    actions: document.getElementById('meetingActions').value.trim()
  };
  if (editingId) {
    const idx = state.meetings.findIndex(m => m.id === editingId);
    if (idx > -1) state.meetings[idx] = { ...state.meetings[idx], ...data };
  } else {
    state.meetings.push({ id: uid(), createdBy: currentUser.id, ...data });
  }
  save();
  closeAllModals();
  renderMeetings();
...
function renderChat() {
  ...
  if (!state.chat.length) {
    area.innerHTML = '<div style="color:var(--text3);text-align:center;padding:60px">No messages yet. Say hi! 👋</div>';
    return;
  }
  ...
  state.chat.forEach(msg => {
    ...
    const initials = sender ? sender.initials : '?';
    const color = sender ? sender.color : '#6366f1';
...
function fileIcon(name) {
  ...
  const map = { pdf:'📄', doc:'📝', docx:'📝', ppt:'📊', pptx:'📊', xls:'📈', xlsx:'📈', jpg:'🖼', jpeg:'🖼', png:'🖼', gif:'🖼', mp4:'🎬', zip:'🗜', rar:'🗜', py:'🐍', js:'⚡', html:'🌐' };
  return map[ext] || '📁';
}

function getUploaderName(id) {
  const m = state.members.find(m => m.id === id);

...
async function load() {
  ...
  // Poll every 10 seconds so team members see each other's updates
  setInterval(async () => {
    try {
...
function uploadFilesToStorage(files) {
  ...
  progressEl.textContent = 'Adding files...';
...
function renderAll() {
  ...
  renderResearch(); renderMeetings(); renderChat(); renderFiles(); updateBadges();
}
...
function renderFiles() {
  ...
  if (!state.files||!state.files.length) { grid.innerHTML=''; return; }
  grid.innerHTML=state.files.map(f=>`
    <div class="file-card">
      ...
      <div class="file-actions">
        ${f.url?`<a class="file-download" href="${f.url}" target="_blank" onclick="event.stopPropagation()">Open ↗</a>`:''}
